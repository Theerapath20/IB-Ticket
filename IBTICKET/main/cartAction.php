...
if(isset($_REQUEST['action']) && !empty($_REQUEST['action'])){
    if($_REQUEST['action'] == 'addToCart' && !empty($_REQUEST['id'])){
		...
		if(empty($address)){
			$errorMsg .= 'กรุณากรอกที่อยู่จัดส่ง.<br/>';
		}
		if(empty($slip)){
			$errorMsg .= 'กรุณาแนบสลิปโอนเงินน.<br/>';
		}
		
		if(empty($errorMsg)){
            // Insert customer data into the database
			$sqlQ = "INSERT INTO customers (first_name,last_name,email,phone,address,created,modified,slip) VALUES (?,?,?,?,?,NOW(),NOW(),?)";
			$stmt = $db->prepare($sqlQ);
			$stmt->bind_param("ssssss", $db_first_name, $db_last_name, $db_email, $db_phone, $db_address,$db_slip,);
			...
			if($insertCust){
                ...
                $sqlQ = "INSERT INTO orders (customer_id,grand_total,created,status_order) VALUES (?,?,NOW(),?)";
                $stmt = $db->prepare($sqlQ);
                $stmt->bind_param("ids", $db_customer_id, $db_grand_total, $db_status_order);
